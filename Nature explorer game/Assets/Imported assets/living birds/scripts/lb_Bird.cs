...
public class lb_Bird : MonoBehaviour {
	...
	void OnGroundBehaviors(){
		...
		if(idle){
			...
			// but only if they are not on the player.
			if (onPlayer) {
				// perched behavior for as long as the player stands still, but no longer than 10 sec
			}
			else if (Random.value < Time.deltaTime*.1){
				FlyAway ();
			}
		}
	}
...
// 	IEnumerator FlyToPlayer(Vector3 target){
// 		if(Random.value < .5){
// 			GetComponent<AudioSource>().PlayOneShot (flyAway1,.1f);
// 		}else{
// 			GetComponent<AudioSource>().PlayOneShot (flyAway2,.1f);
...
// 						if(!hit.collider.isTrigger){
// 							AbortFlyToTarget();
// 							Debug.Log("Flight to player aborted because of collision.");
// 						}
// 					}
// 				}
